Fixture for the reader-map firing tests.
`design-note.md` is a design note whose sentences each parse and whose headings read as an outline,
and which still loses its reader:
the spool file is described by its properties for two sentences before the third says what it is,
a paragraph about the spool reappears inside the section on deduplication with nothing to say it has gone back,
...
defaults and one-line descriptions — for the scenarios that must not fire.
